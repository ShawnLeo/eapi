...
<template>
    <div class="search">
        <Row>
            <Card>
                <Row class="operation">
                    ...
                    <Button @click="add" type="primary" icon="md-add" style="margin-right:10px">添加字段</Button>
                    <Button @click="generate" :loading="loading" type="info" icon="md-send" style="margin-right:10px">
                        生成代码
                    </Button>
                    <Button @click="entityModal=true" type="warning" icon="md-code-working" style="margin-right:10px">
                        根据JSON配置
                    </Button>
                    ...
                    <Table border :columns="columns" :data="data" ref="table">
                        ...
                        <template slot-scope="{ row }" slot="type">
                            ...
                            >
                                ...
                                <Option value="upload">图片上传组件</Option>
        ...
        <Modal title="根据JSON配置" v-model="entityModal" :width="700" :fullscreen="fullEntity">
            <Alert show-icon>说明：输入JSON,将读取其字段,装换为生成Vue代码的配置JSON数据，免去手动配置添加字段</Alert>
            <Form ref="entityForm" :model="entityForm" :label-width="110">
                <ace-editor v-model="entityForm.json" lang="json" height="250"
            ...
            <div slot="footer">
                <Row type="flex" justify="center" class="code-row-bg">
                    <i-col span="8">
                        <Button @click="fullEntity=!fullEntity" icon="md-expand" style="width:150px;padding: 9px 0px">
                            全屏开/关
                        </Button>
                    </i-col>
                    <i-col span="8">
                        <Button type="warning" icon="md-code-working" @click="generateEntityData"
                                style="width:150px;padding: 9px 0px">根据JSON生成
                        </Button>
                    </i-col>
                    <i-col span="8">
                        <Button type="primary" icon="md-checkmark-circle-outline"
                                @click="submitEntityData" style="width:150px;padding: 9px 0px">
                            确认更新字段值
                        </Button>
                    </i-col>
                </Row>
...
<script>
    import {generateTable, getEntityData} from "../../../utils/interface";
    ...
    export default {
        ...
        methods: {
            init() {
                // 取缓存数据
                let data = getStore("tableData");
                let vueName = getStore("tableVueName");
                let rowNum = getStore("tableRowNum");
                let defaultSort = getStore("tableDefaultSort");
                let daterangeSearch = getStore("tableDaterangeSearch");
            ...
            generateEntityData() {
                if (this.entityForm.json) {
                    getEntityData(this.entityForm, res => {
                        this.entityData = res.body;
                    });
                } else {
                    this.$Message.warning("请先输入JSON数据");
                }
            },
            submitEntityData() {
                if (!this.entityData) {
            ...
            generate() {
                ...
                this.loading = true;
                generateTable(this.vueName, this.rowNum, this.data,res => {
                    this.loading = false;
                    this.code = res.body;
                    this.codeModal = true;
                    this.save();
            ...
            save() {
                setStore("tableData", JSON.stringify(this.data));
                setStore("tableVueName", this.vueName);
                setStore("tableRowNum", this.rowNum);
                setStore("tableDefaultSort", this.defaultSort);
                setStore("tableDaterangeSearch", this.daterangeSearch);
            }
        },
        mounted() {
            this.init();
