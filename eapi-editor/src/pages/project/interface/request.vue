      ...
      <span slot="label">
        ...
        <Tooltip content="body和formData不可同时选择" placement="right">
          ...
          <span>
			...
			<Table :columns="bodyColumns" :data="interfaceItem.body" v-if="requestType === 'body'"></Table>
			<Table :columns="formDatasColumns" :data="interfaceItem.formDatas"
				v-if="requestType === 'formData'"></Table>
			<import-data v-on:childrenChange="formChildrenChange"
				v-if="requestType === 'formData'"></import-data>
						...
						render: (h, params) => {
							return h('div', [
								h('i-switch', {
									...
									on: {
										'on-change': (value) => {
											this.interfaceItem.body[params.index].required = value;
											this.interfaceItem.body[params.index].dataModel.required = value;
										}
									}
								})
							]);
...
//						this.initBody();
			...
			requestTypeChange(value) {
				let exitsData = false;
				if (value === "formData") {
					exitsData = this.interfaceItem.body && this.interfaceItem.body.length > 0 && this.interfaceItem.body[0].dataModel.name;
				} else if (value === "body") {
					exitsData = this.interfaceItem.formDatas && this.interfaceItem.formDatas.length > 0;
				}

				if (!exitsData) { // 没有数据直接切换
					value === "body" && this.initBody(); //  body只允许有一个，初始化一条
					this.requestType = this.interfaceItem.requestType;
					return;
				}
